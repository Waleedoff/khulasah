  ...
  return (
    ...
    >
      ...
      <button
        disabled={disabled}
        className={`flex ${
          smallToggle ? "h-5 w-8 " : "h-6 w-11"
        } cursor-pointer items-center rounded-2xl p-0.5 transition-all duration-300 disabled:cursor-not-allowed ${
      ...
      >
        <div
          className={`${
            ...
            smallToggle ? "h-4 w-4" : "h-5 w-5"
          } items-center justify-center rounded-full bg-white text-xs`}
        ></div>
      </button>
      <span
        className={`${textSmall ? "text-base" : "text-xl"} text-neutral-600`}
